...
async function searchBooks() {
  ...
  if (!q) { await loadBooks(currentGenre); return; }
...
function initAuthModal() {
  ...
  const updateButton = () => { openBtn.textContent = getToken() ? "Вы вошли" : "Войти"; };
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    try {
      const body = {email: document.getElementById("loginEmail").value.trim(), password: document.getElementById("loginPassword").value.trim()};
      const result = await apiFetch("/auth/login", {method: "POST", body: JSON.stringify(body)});
